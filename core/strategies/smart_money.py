...
class SmartMoneyStrategy(BaseStrategy):
    def __init__(self, name: str, mt5_client, logger: logging.Logger, database=None):
        """
        Инициализация стратегии "Смарт Мани"
        ...
        # Проверка параметров
        if not all([self.sma_period > 0, self.ema_period > 0, self.rsi_period > 0,
                    self.atr_period > 0, self.volume_ma_period > 0]):
            raise ValueError("Все периоды индикаторов должны быть положительными")
